...
const VARS = {
  primary:  {background:"var(--qg-gold)",color:"#FFFFFF",  border:"none",            borderRadius:"100px"},
...
const HOVER_BG = { primary:"var(--qg-primary-hover)" };
export default function Btn({ onClick, children, v = "primary", sz = "md", dis, sx = {} }) {
  const [hov, setHov] = useState(false);
  const base = VARS[v] || VARS.primary;
  const hovStyle = hov && !dis && HOVER_BG[v] ? { background: HOVER_BG[v] } : null;
  return (
    <button
      onClick={dis ? undefined : onClick}
      onMouseEnter={() => setHov(true)}
      onMouseLeave={() => setHov(false)}
      style={{display:"inline-flex",alignItems:"center",borderRadius:DS.r2,cursor:dis?"not-allowed":"pointer",fontWeight:600,opacity:dis?.5:1,fontFamily:"inherit",transition:"all .15s",...SZ[sz],...base,...hovStyle,...sx}}
    >
      {children}
    </button>
  );
