    ...
    <div style={{height:"100vh",display:"flex",background:DS.bg}}>
      ...
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:48}}>
        <div style={{maxWidth:500,width:"100%"}}>
          <div style={{fontSize:10,fontWeight:700,color:"#2563EB",letterSpacing:1,marginBottom:8,textTransform:"uppercase"}}>Step {step+1} of 3</div>
          ...
          {step===0 && <>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              {ORGS.map(o=>(
                <button key={o} onClick={()=>setOrg(o)} style={{padding:"14px",borderRadius:DS.r2,border:`2px solid ${org===o?"#2563EB":DS.bd}`,background:org===o?"rgba(37,99,235,0.10)":DS.surface,cursor:"pointer",textAlign:"left",fontFamily:"inherit"}}>
                  <div style={{fontSize:12,fontWeight:700,color:org===o?"#2563EB":DS.t1}}>{o}</div>
                </button>
              ))}
            </div>
          ...
          {step===1 && <>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:9}}>
              {SRCS.map(sr=>(
                <button key={sr.l} onClick={()=>setSrc(sr.l)} style={{padding:"13px 16px",borderRadius:DS.r2,border:`2px solid ${src===sr.l?"#2563EB":DS.bd}`,background:src===sr.l?"rgba(37,99,235,0.10)":DS.surface,cursor:"pointer",display:"flex",alignItems:"center",gap:12,fontFamily:"inherit"}}>
                  <span style={{fontSize:20,flexShrink:0}}>{sr.e}</span>
                  <div style={{textAlign:"left",flex:1}}>
                    <div style={{fontSize:13,fontWeight:700,color:src===sr.l?"#2563EB":DS.t1}}>{sr.l}</div>
                    <div style={{fontSize:10,color:DS.t3}}>{sr.d}</div>
                  </div>
                  {src===sr.l&&<span style={{color:"#2563EB",fontWeight:800,marginLeft:"auto"}}>✓</span>}
                </button>
              ))}
            </div>
          ...
          {step===2 && <>
            ...
            <Card sx={{background:"rgba(37,99,235,0.06)",border:"1px solid rgba(37,99,235,0.20)",marginBottom:18}}>
              <div style={{fontSize:13,fontWeight:700,color:"#2563EB",marginBottom:8}}>📋 Pre-configured for your organization</div>
