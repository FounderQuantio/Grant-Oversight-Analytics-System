...
function LineChart({ totalData, flaggedData, labels }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" height="100%" style={{ display: "block" }}>
      ...
      <path d={areaPath} fill="rgba(61,90,153,0.10)" />
      <polyline points={totalPts} fill="none" stroke="#3D5A99" strokeWidth={2} strokeLinejoin="round" />
      {totalData.map((v, i) => (
        <circle key={i} cx={xp(i)} cy={yp(v)} r={3}
          style={{ fill: "var(--qg-surface)" }} stroke="#3D5A99" strokeWidth={1.5} />
      ))}
          ...
          <div style={{ display: "flex", border: `1px solid ${T.border}`, borderRadius: 8, overflow: "hidden", background: T.card }}>
            {["30d", "90d", "YTD"].map(r => (
              <button key={r} onClick={() => setTimeRange(r)} style={{
                ...
                background: timeRange === r ? "#3D5A99" : "transparent",
          ...
          {DS_FILTERS.map(f => {
            ...
            return (
              <button key={String(f.id)} onClick={() => setActiveDS(f.id)} style={{
                padding: "5px 14px", borderRadius: 20, border: `1px solid ${isActive ? "#3D5A99" : T.border}`,
                background: isActive ? "#3D5A99" : T.pillBg,
        ...
        <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, padding: "20px 20px 12px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", gap: 16, fontSize: 11, color: T.t3 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                <svg width="20" height="2" style={{ display: "inline-block" }}><line x1="0" y1="1" x2="20" y2="1" stroke="#3D5A99" strokeWidth="2"/></svg>
                Total
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        ...
        <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, padding: 20 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            ...
            <span style={{ fontSize: 12, color: "#3D5A99", cursor: "pointer", fontWeight: 600 }}>View all →</span>
          </div>
          {topVendors.length === 0 && (
